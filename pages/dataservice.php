...
<link rel="stylesheet" href="public/assets/css/dataservice.css">
...
<div style="padding: 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 8px;">
        <div style="display: flex; align-items: center; gap: 8px;">
            ...
            <div style="display: flex; align-items: center; gap: 6px; padding: 7px 12px; border: 1px solid #34a0a4; border-radius: 8px; background: #FFFFFF; font-size: 12px; color: #1e6091;">
                ...
                <input
                    placeholder="Cari..."
                    style="border: none; outline: none; font-size: 12px; color: #1e6091; width: 120px; background: transparent;"
                />
            </div>
        </div>
    ...
    <div style="background: #FFFFFF; border: 1px solid #34a0a4; border-radius: 12px; overflow: hidden;">
                ...
                <tbody>
                    <?php foreach ($mockDataService as $s): ?>
                        <tr>
                            <td>
                                <span
                                    style="font-family: monospace; font-size: 12px; color: <?= $COLORS['amber'] ?>; font-weight: 600;"
                                >
                            ...
                            <td>
                                <div style="font-weight: 600; color: #184e77;">
                                    <?= htmlspecialchars($s['pelanggan']) ?>
                                </div>
                            </td>

                            <td><?= htmlspecialchars($s['kendaraan']) ?></td>

                            <td>
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <div
                                        style="width: 24px; height: 24px; border-radius: 50%; background: #b5e48c; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: <?= $COLORS['amber'] ?>;"
                                    >
                                        <?php
                                        $m_parts = explode(' ', $s['mekanik']);
                                        echo htmlspecialchars(substr(end($m_parts), 0, 1));
                                        ?>
                                    </div>

                                    <?= htmlspecialchars($s['mekanik']) ?>
                                </div>
                            </td>

                            <td><?= htmlspecialchars($s['layanan']) ?></td>
                            <td style="font-size: 12px;"><?= htmlspecialchars($s['mulai']) ?></td>
                            <td style="font-size: 12px;"><?= htmlspecialchars($s['selesai']) ?></td>
        ...
        <div style="padding: 12px 16px; border-top: 1px solid #eee; background: #f6f5f7;">
            <p style="font-size: 12px; color: #52b69a; margin: 0;">
                Menampilkan <?= count($mockDataService) ?> data service
            </p>
        </div>
    </div>
</div>
