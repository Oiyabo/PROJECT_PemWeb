...
class PendingModel
{
    ...
    public function konfirmasiPembayaranFull(
        ...
        if ($ok && $localStatus === 'Selesai') {
            ...
            if ($row) {
                // We need to call upsertHubung. We can just execute the stored procedure.
                $stmtUpsert = $this->db->prepare('CALL sp_upsert_hubung(?, ?, ?)');
                $stmtUpsert->execute([ (int) $row['id_reservasi'], null, (int) $row['id_pembayaran_full'] ]);
    ...
    public function prosesNotifikasiMidtrans(array $notif): array
    {
        ...
        if ($tipe === 'FULL') {
            ...
            if ($dp && !empty($dp['id_reservasi'])) {
                $stmtUpsert = $this->db->prepare('CALL sp_upsert_hubung(?, ?, ?)');
                $stmtUpsert->execute([ (int) $dp['id_reservasi'], (int) $dp['id_pembayaran_dp'], null ]);
                $stmtUpsert->closeCursor();
            }
        }
